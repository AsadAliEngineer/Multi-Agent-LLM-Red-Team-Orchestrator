...
export default function Lesson() {
  return (
    <>
      ...
      <p>Skills reduce attack surface on one side. They add it on another. The same traits that make a skill useful (reusable, team-shared, loaded at runtime) also make it a target. An attacker who poisons your skill gets code or instructions running inside every agent that loads it. This lesson covers the two main skill attack vectors confirmed in the wider 52-scenario test catalog.</p>
      ...
      <p><AttackRef id="SS1" /> (a skill makes the agent install a second attacker-controlled skill, spreading attacker behavior to new agent contexts) is the skill-layer version of the <AttackRef id="SP1-FC" /> worm. The attacker's skill copies itself.</p>

      <AttackCard
        id="SS1"
        model="Any agent loading the skill"
        title="Skill Worm (Self-Propagating Skill)"
        mechanism="A malicious skill (installed via SC2 or a compromised PR) contains an instruction to load a second skill from an attacker-controlled source. Any agent that loads the first skill and has write access to the shared skills folder writes the new skill there. Each infected agent spreads to shared infrastructure on its next run. The attacker takes no further action after the first submission."
        impact="Attacker behavior spreads across all agents sharing the same skill repository. A cross-session, cross-agent worm effect, the same shape as the SP1-FC worm but at the skill layer. One registry submission poisons the entire shared agent environment."
      />
      ...
      <Callout type="warn" title="If you would not run a random npm package without checking it, do not load a random skill without checking it.">
        npm packages get code review, dependency audits, and pinned version hashes before they run in production. Skills deserve the same treatment. A skill is code, or instruction text that directs code. Both are executable from an attacker's point of view.
      </Callout>
      ...
      <KeyPoint>
        Skills from internal, signed, pinned sources deserve higher trust. Skills from public registries, shared community folders, or external vendors are attacker-reachable input. Treat them exactly as you treat public form submissions, community posts, and vendor documents: review them before use.
      </KeyPoint>
    </>
  );
}
