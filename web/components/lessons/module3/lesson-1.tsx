...
export default function Lesson() {
  return (
    <>
      ...
      <FlowSteps steps={[
        ...
        { icon: "note", label: "Clean NDA (C)", desc: "No attack here. It just references the vendor policy on file" },
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>Changed to a clean company subdomain, so there was no mismatch</td>
          </tr>
          <tr>
            <td>The "last verified" date was months in the past</td>
            <td>Updated to today's date, so there was no freshness warning</td>
          </tr>
        </tbody>
      </table>
      ...
      <AttackCard
        ...
        mechanism="The attacker has edit access to a shared vendor NDA page. That is the kind of access any vendor representative would request. They write a 'Paralegal Notes' section as a transcribed phone conversation. The AI fetches the page through its connected tool and reads the note as a trusted human record, not an instruction. The operator's own instruction, 'include all paralegal notes,' gives full license to propagate the URL."
  ...
  nodes={[
    ...
    { type: "source", label: "Shared Page (connected tool output)", detail: "No instruction-looking text, just pure conversational prose. The tool returns it as trusted data" },
    { type: "agent", label: "Legal AI Agent", detail: "Instructions given to the AI: 'include all paralegal notes and vendor requirements'" },
    { type: "artifact", label: "Workflow Checklist", detail: "Attacker URL passed along as a real checklist step, with no warnings" },
