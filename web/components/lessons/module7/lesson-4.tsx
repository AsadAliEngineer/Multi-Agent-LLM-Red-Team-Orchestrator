...
export default function Lesson() {
  return (
    <>
      ...
      <p>This lesson shows what a well-built defensive skill actually looks like. The goal is not perfection. It is replacing soft text reminders with enforceable code gates at the right decision points.</p>
    ...
    main()`}</code></pre>
      ...
      <DoDont
        do={[
          ...
          "Wire skill invocation at the orchestrator layer so the write API call does not fire unless the skill returns OK",
          "Run quarterly skill audits on the same cadence as your prompt audits"
        ]}
        dont={[
          "Write a text-only skill that says 'please verify URLs before writing.' Attackers beat this with competing commands",
          "Allow agents to pick which skills to invoke. The required call must be wired at the orchestrator",
          "Trust an unversioned skill file in a shared folder. Who changed it last, and when?",
          "Skip the quarterly audit because nothing has changed. Code controls need regular checks too"
        ]}
      />

      <h2>Signing and Pinning</h2>

      <p>Unsigned skills are in the same trust class as unsigned npm packages: maybe fine, maybe not, and you cannot tell the difference at load time.</p>
      ...
      <KeyPoint>
        Skills that call code are a real defense. Skills that are pure text barely help. Always signed, always pinned, always audited, and wired at the orchestrator so the model cannot skip the check even under attack. That is the difference between a security control and a security intention.
      </KeyPoint>
    </>
  );
}
