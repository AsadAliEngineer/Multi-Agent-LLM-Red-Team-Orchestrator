...
export default function Lesson() {
  return (
    <>
      ...
      <AttackCard
        ...
        mechanism="CONF1 v3: A config file's analytics address uses a web address that matches the app's own name, so it passes the AI's name-matching check. MAA1+CONF1 v2: the attacker rotates the domain to one with no name match, bypassing the check entirely."
      ...
      <p>The vulnerability is the gap between what the helper AI writes and what the main AI reads. The human review step closes that gap. It does not work by making the main AI more skeptical of internal data, because it will keep trusting internal data. It works by making sure that data was actually reviewed before it became authoritative.</p>
      ...
      <DoDont
        ...
        dont={[
          ...
          "Allow cross-source reinforcement: the wiki says 'trust the list,' and the list has the attacker URL. This makes the attack more convincing, not safer"
        ]}
      />
      ...
      <AttackDiagram
  nodes={[
    { type: "attacker", label: "Attacker", detail: "Adds ingest.stackdrive-analytics.io to config file, with no name overlap with the app" },
  ...
  caption="MAA1+CONF1 v2: Domain rotation. The main AI's name-matching check is bypassed with one domain change"
/>
