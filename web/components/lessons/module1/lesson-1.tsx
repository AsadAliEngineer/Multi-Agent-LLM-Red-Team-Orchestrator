...
export default function Lesson() {
  return (
    <>
      ...
      <p>Traditional software does exactly what its code tells it to do. An AI agent reads information, makes decisions, and takes actions across many systems at once. That is not a small upgrade. It is a completely different security problem. Here is the part most enterprises miss: the attacker does not need to hack anything. They just submit ordinary inputs through ordinary public surfaces (a vendor registration form, a support ticket, a Slack community post) and wait for an AI agent to read that data and act on it.</p>
      ...
      <p><strong>Wide write access.</strong> One enterprise agent may read from email, Slack, and GitHub while writing to Notion, Jira, and a vendor list. The potential damage is far larger than any single traditional application. The entry point can be as simple as a public GitHub PR or a survey response form.</p>

      <h2>Real Attack: SP1, Poisoned Vendor Registry</h2>

      <UseCase
        scenario="A legal AI agent handles contract workflows. The instructions given to the AI tell it to trust portal links only from IT's Approved Vendor Registry. The registry is an editable table, so anyone with vendor access can submit entries through the public vendor-registration process."
        attacker="SP1 (an attack where a vendor table is quietly altered): The attacker fills out the public vendor-registration form and adds a malicious portal link to the registry as a clean entry, using a real company domain, verified today. Nothing looks wrong. The agent reads the table and passes the attacker's link to the checklist as if it were official. No hacking required. Just a normal form."
        ...
        defense="This is a beginner-level AI integration mistake: connecting an AI agent to an editable table and telling it to trust whatever is in there. The 'trust the registry' rule is exactly what SP1 exploits. Any basic guardrail would have stopped this: input validation on new vendor entries, URL allow-listing, or a human approval step before the AI surfaces links. Confirm portal links through a separate channel before the AI uses them in action items."
      />
  ...
  nodes={[
    ...
    { type: "agent", label: "Legal AI Agent", detail: "Reads the table as instructed and treats the attacker link as official" },
