...
export default function Lesson() {
  return (
    <>
      ...
      <p><strong>Signal 2: List or catalog updated without a human approval record.</strong> Every write to an authoritative source should have a matching human approval entry in the audit log. An update with no approval record is a mandatory trigger, no matter how routine the content looks.</p>
      ...
      <p><strong>Signal 4: Sources do not match the stated task.</strong> If the log shows the AI reading a deployment pipeline log when the user only asked for an NDA summary, the AI consumed sources outside the user's intended scope. This is why you track where data came from.</p>
      ...
      <UseCase
        ...
        impact="Without monitoring: link written to the live deployment guide, engineers follow it at next deployment. With link checker: deploy-verify.acmecorp.com is not on the approved list, so it is flagged before writing, and a human reviewer sees the log context and rejects it."
  ...
  nodes={[
    ...
    { type: "artifact", label: "Deployment Guide / Alert", detail: "Without defense: link in guide. With link checker: flagged, and a human reviewer sees log context and rejects" },
