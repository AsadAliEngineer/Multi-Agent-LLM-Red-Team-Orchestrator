...
export default function Lesson() {
  return (
    <>
      ...
      <Callout type="info">
        These numbers reflect attacks tested against each model. Sonnet was the main test target, so not all attacks were run against Opus. Confirmed Opus bypasses: <AttackRef id="SP1" />, <AttackRef id="WIKI1 v4" />, <AttackRef id="MAA1" />+<AttackRef id="CONF1" /> v2, <AttackRef id="SURV1 v2" />, and a <AttackRef id="TP1 v3" /> equivalent.
      </Callout>
...
/>
      ...
      <Callout type="danger" title="Opus is more vulnerable to table and list attacks than Sonnet">
        Opus is better at catching attacks in document text, so it relies more heavily on tables and approved lists as ground truth. When a list says an endpoint is approved, Opus trusts it more than Sonnet does, because Opus has already learned to distrust document prose. Stronger document defenses create a new blind spot for lists. Better in one area, worse in another. The entry point for both is the same: a normal public form or PR.
      </Callout>
